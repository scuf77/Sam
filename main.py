...
async def cmd_start(message: Message, state: FSMContext):
    ...
    text = (
        ...
        "✨ Мы создаем самые <i>вкусные</i> и <i>красивые</i> торты на заказ!\n\n"
        "🍰 <b>Наши преимущества:</b>\n"
        "• <i>Свежие</i> ингредиенты\n"
        "• <i>Домашние</i> рецепты\n"
        "• <i>Быстрая</i> доставка\n"
        "• <i>Индивидуальный</i> подход\n\n"
        "🎯 <b>Что вы можете сделать:</b>\n"
        "• Выбрать торт из <code>каталога</code>\n"
...
async def back_handler(callback: CallbackQuery):
    ...
    if action == "main":
        text = (
            ...
            "🔄 <b>Перезапуск</b> - сбросить все данные и начать заново\n\n"
...
Мы очень <i>ценим</i> мнение каждого клиента! Здесь вы можете:

📝 <b>Оставить свой отзыв</b> - поделитесь <i>впечатлениями</i> о заказе
⭐ <b>Почитать отзывы</b> - узнайте, что <i>говорят</i> другие клиенты
...
async def restart_bot(message: Message, state: FSMContext):
    """Перезапускает бота - очищает состояние и показывает главное меню"""
    await state.clear()
...
async def main():
    ...
    dp.message.register(restart_bot, F.text == "🔄 Перезапуск")
